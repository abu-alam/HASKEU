...
{-tcRho (Con (Special Cons)) expTy = do 
...
tcPat (PApp k ps) expTy = do
  ...
  --check (isSimple resTy) $ text $ "tcPat: Constructor given too few parameters: " ++ prettyPrint k -- Abu
...
tcPat p@(PInfixApp (PVar n1) (Special Cons) (PVar n2)) expTy = do
  ...
  q2 <- instSigma listDataCon expTy

  q3 <- instSigma (listType ty1) expTy
  return ([(UnQual n1, ty1), (Special Cons, listDataCon), (UnQual n2, (listType ty1))], (q1 ++ q2 ++ q3), [])
...
{-tcPat p@(PList []) expTy = do
  --ty <- listCon 
  q <- instSigma listCon expTy -- q will be []
  return ([], q, [])-}
