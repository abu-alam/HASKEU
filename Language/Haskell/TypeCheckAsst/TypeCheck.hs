module Language.Haskell.TypeCheckAsst.TypeCheck where

import Language.Haskell.TypeCheck.InternalTypes

import Language.Haskell.TypeCheckAsst.Monad
import Language.Haskell.TypeCheckAsst.Simplify
...
--     The top-top-level                    --
----------------------------------------------
-- other module env, undefined env in this module, module
getAllVarEnv :: ([(QName, Sigma)], [(QName, Sigma)]) -> ([(QName, Sigma)], [(QName, Sigma)]) ->
		 Module -> IO ([(QName, Sigma)], [(QName, Sigma)])
getAllVarEnv (embeddedVar, tempEmbedVar) (moduleVar, tempVar) mod = 
	do	
	let Tc tc = typecheckModule mod
	env <- mkTcEnv (embeddedVar ++ tempEmbedVar ++ moduleVar ++ tempVar) [] []
	tcRes <- tc env
	...
	case tcRes of
		Left  e -> 
			--do
			--putStrLn (show e)
			getAllVarEnv (embeddedVar, tempEmbedVar) (moduleVar, (tempVar ++ (mkTempVar $ show e))) mod
		Right e -> return (moduleVar ++ e, tempVar)

mkTempVar n = 
	let var :: [(QName, Sigma)]
	    var  = [(getUnqual $ words n, TcTyVar (BoundTv n))] --should not be n
	in var

getUnqual lst =
	case ((lst!!0), (lst!!1)) of
		("UnQual", "Ident")  -> UnQual (Ident (lst!!2))
		("UnQual", "Symbol") -> UnQual (Symbol (lst!!2))
		("Special", "Cons")  -> Special Cons
		("Special", "ListCon")  -> Special ListCon
		("Special", "UnitCon")  -> Special UnitCon
		(_, _)  -> UnQual (Ident "")
...
typecheckModule (Module _ _ _ _ _ _ ds) = do
  ...
  case qRem of
    [] -> return ve
    q  -> return ve--fail $ "Unsolved assertions: " ++ show q
...
tcRho (List []) expTy = do 
  --instSigma (TcTyVar (BoundTv "[]")) expTy
  let	sig = TcForAll [TcTyVarBind (BoundTv "a") (TcTyVar $ BoundTv "a")] [] (TcTyApp (TcTyCon $ Special ListCon) (TcTyVar $ BoundTv "a")) 
  instSigma sig expTy

--tcRho e _ = return []-- fail $ "Unsupported expression: " ++ prettyPrint e
tcRho _ expTy = do 
  let	sig = TcForAll [TcTyVarBind (BoundTv "a") (TcTyVar $ BoundTv "a")] [] (TcTyApp (TcTyCon $ Special ListCon) (TcTyVar $ BoundTv "a")) 
  instSigma sig expTy
...
inferGuardedAlts galts qDis = do
  ref <- newTcRef $ error "inferAlt: empty result--gur"
...
tcPat p@(PInfixApp (PVar n1) (Special Cons) (PVar n2)) expTy = do
  lift $ putStrLn ("PInfixApp" ++ (show p))
  -- No lookup - this is a definition!
  ty1 <- newTyVarTy
  q1 <- instSigma ty1 expTy
  ty2 <- newTyVarTy
  q2 <- instSigma ty2 expTy
  ty3 <- newTyVarTy
  q3 <- instSigma ty3 expTy
  return ([(UnQual n1, ty1), (Special Cons, ty2), (UnQual n2, ty3)], (q1 ++ q2 ++ q3), [])

{-tcPat p@(PInfixApp p1@(PVar n1) (Special Cons) p2@(PVar n2)) expTy = do
  ty <- newTyVarTy
  q <- instSigma ty expTy 
  ty2 <- newTyVarTy
  q2 <- instSigma ty2 expTy
  return ([(UnQual n1, ty)] ++ [(Special Cons, listA ty)] ++ [(UnQual n2, listA ty)], [], [])-}

tcPat p@(PList []) expTy = do
  ty <- newTyVarTy
  q <- instSigma ty expTy -- q will be []
  return ([(Special ListCon, listCon)], q, [])
...
listDataCon = TcTyCon $ Special Cons
listA a = TcTyApp listCon (TcTyVar $ BoundTv (show a))
