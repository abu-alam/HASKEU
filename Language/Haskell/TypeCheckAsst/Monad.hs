{-# LANGUAGE RankNTypes #-}
module Language.Haskell.TypeCheckAsst.Monad where
...
lookupAux f n = do m <- getEnvField f
                   case Map.lookup n m of
                     ...
                     Nothing -> failTc (strQName n)

strQName (UnQual (Ident s)) = text $ "UnQual Ident " ++ s
strQName (UnQual (Symbol s)) = text $ "UnQual Symbol " ++ s
strQName (Special Cons) = text $ "Special Cons"
strQName (Special ListCon) = text $ "Special ListCon"
strQName (Special UnitCon) = text $ "Special UnitCon"
strQName _ = text $ "Unidentified Qual"
...
unify ty1 ty2 | badType ty1 || badType ty2 -- Compiler error
                  = return () --failTc (text "Panic! Unexpected types in unification:" <+> vcat [ppr ty1, ppr ty2])
...
unify ty1 ty2 = return () --failTc (text "Cannot unify types:" <+> vcat [ppr ty1, ppr ty2])
...
occursCheckErr tv ty 
    = return () --failTc (text "Occurs check for" <+> quotes (ppr tv) <+> text "in:" <+> ppr ty)
